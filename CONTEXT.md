...
### Programme Delivery and Bounded Autonomy

**Reviewed Source Candidate**:
An exactly identified body of source changes supported by the required source review and applicable check evidence. It is distinct from an integrated or installed qualified release.
_Avoid_: Installed system, production delivery, programme complete

**Milestone Budget**:
A finite authorized resource envelope shared by a milestone and its supporting work, including retries, review, integration and recovery. Currency is one possible unit; unknown prices do not imply unbounded resources.
_Avoid_: Per-job approval, free request, unlimited subscription

**Effect-Free Inference Attempt**:
An inference attempt whose execution capabilities demonstrably exclude tools and external effects beyond the inference request and its resource consumption.
_Avoid_: Tool-capable agent, presumed read-only worker, free call

**Replacement Inference Attempt**:
A separately identified, bounded inference attempt replacing an unresolved effect-free attempt while preserving the original uncertainty and resource accounting. Alternative or late results do not represent multiple accepted outcomes for the same logical result.
_Avoid_: Replay, confirmed absence, duplicate acceptance

**Goal Scope Disposition**:
An explicit owner decision removing or replacing an identified obligation within a goal, with its reason and surviving responsibility recorded. Deferral and blockage preserve outstanding obligations.
_Avoid_: Completion receipt, silent deferral, status correction
