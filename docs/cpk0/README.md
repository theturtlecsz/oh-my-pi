...
Baseline performance numbers for the coding-agent CLI are measured by `scripts/cpk0-baseline.ts` and committed at [`baseline.json`](./baseline.json). These figures establish regression budgets for cold-boot latency, memory overhead, and default system prompt tokens across the CPK-0 kernel/plugin boundary architecture.

### Schema and Field Reference

| Field | Type | Description |
|---|---|---|
| `schema_version` | `string` | Schema identifier (`"cpk0-baseline/v1"`). |
| `method` | `object` | Descriptions of the three measurement methodologies (`startup`, `memory`, `tokens`). |
| `observed` | `object` | Frozen baseline measurements across 5 cold-boot samples (`startup_latency_ms`, `memory_overhead_mb`, `system_prompt_tokens`, `runs`). |
| `budgets` | `object` | Maximum allowable thresholds: `ceil(observed * 1.5)` for latency and memory (50% headroom), `ceil(observed * 1.10)` for system prompt tokens (10% headroom). |
| `platform` | `string` | Operating system platform on which the baseline was frozen (e.g. `"linux"`). |

#### Observed and Budget Metrics

- `startup_latency_ms` (`number`): Median cold-boot wall time (process spawn to exit) of `bun --preload scripts/cpk0-rss-preload.ts packages/coding-agent/src/cli.ts` under `PI_TIMING=x`, which runs the full CLI pre-paint chain and exits. Each run uses a fresh, isolated `HOME` and `cwd`.
- `memory_overhead_mb` (`number`): Median CLI RSS minus bare Bun runtime RSS (`bun --preload scripts/cpk0-rss-preload.ts -e 0`), isolating the coding agent's heap and module footprint from the underlying Bun runtime floor.
- `system_prompt_tokens` (`number`): Strict native token count of the default system prompt generated by `buildSystemPrompt` against an empty workspace.
- `runs` (`number`): Number of CLI and bare runtime sample iterations used to compute the medians (default: 5).

### Host Dependence and Budget Rationale

Startup latency and memory overhead are inherently machine- and hardware-dependent: they vary based on CPU architecture and clock speed, storage I/O throughput, kernel memory allocation, and host load. In contrast, the system prompt token count is deterministic across platforms.

To account for host variance without risking false-positive regressions in development and CI environments:
- `startup_latency_ms` budget is set to `ceil(observed.startup_latency_ms * 1.5)` (50% headroom).
- `memory_overhead_mb` budget is set to `ceil(observed.memory_overhead_mb * 1.5)` (50% headroom).
- `system_prompt_tokens` budget is set to `ceil(observed.system_prompt_tokens * 1.10)` (10% headroom to accommodate prompt changes while preventing unbounded prompt inflation).

### Enforcement via `--check`

The committed budgets are enforced using the `--check` flag:

```bash
# Check current system performance against committed docs/cpk0/baseline.json budgets
bun scripts/cpk0-baseline.ts --check

# Check against a custom budget file
bun scripts/cpk0-baseline.ts --check --budget path/to/budget.json
```

For each metric, `--check` prints one line formatted as:
```text
metric observed/budget OK|OVER
```

If any metric exceeds its budget threshold (`observed > budget`), the script exits with non-zero status code (`1`). If all metrics are within budget, it exits with status code `0`.

### Re-freezing the Baseline

When substantial architectural changes or dependency upgrades shift the performance characteristics intentionally, the baseline can be re-measured and re-frozen:

```bash
# Run 5 cold-boot iterations on Linux and write updated docs/cpk0/baseline.json
