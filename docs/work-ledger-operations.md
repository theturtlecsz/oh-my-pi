...
- **Deadline overrun**: if activation has never been submitted, T+45 marks the window failed, removes the freeze marker, invalidates the candidate, and requires a fresh rehearsal 2 before retry. If activation may have landed, Linear stays frozen. After activation but before the first WorkService mutation, use `ops cutover rollback`; after the first mutation, re-run the same `ops cutover execute` command, which resumes post-write recovery without enforcing the elapsed wall-clock ceiling.
- **Credential revocation failure**: a refused `ops cutover finalize` leaves the epoch `active`, Linear frozen, the Work selector installed, and `~/.config/linear.env` intact. Revoke the personal key in Linear settings, then rerun `ops cutover finalize`; do not remove the marker or restore the Linear selector.
