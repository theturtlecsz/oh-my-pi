...
| :--- | :--- | :--- |
| Coordinator | Codex | `gpt-5.6-sol`, effort `medium` |
| Blocker diagnostician | Codex | `gpt-6-astra`, effort `high` |
...
- Planner, worker, and reviewer execute through their installed external CLIs and must never be replaced with native Codex subagents. Coordinator always remains `gpt-5.6-sol` at medium effort throughout the entire workflow, including blocker handling.
...
- A verified economy-worker blocker may use one fresh Astra high diagnostic invocation. Supply one named unresolved question, exact failure evidence and attempted fixes; cap output at 8,000 tokens; expire reserve with current milestone. Astra diagnoses only and returns its recommendation to coordinator and same worker context. It cannot implement, review, accept, broaden scope, reset attempts, or become coordinator.
- Other premium consultation requires a named unresolved question, concrete evidence, separately configured reserve, and bounded envelopes.
...
Read current `AGENTS.md`, mandatory Antidote instructions, Task Observer session-start, `/home/thetu/.codex/workflows/economy/POLICY.md`, and `MODELS.md`. Exact roles: coordinator always uses Codex `gpt-5.6-sol` at medium effort for the entire workflow, including blocker handling; planning uses Fable 5.1 through Claude CLI; mechanical/stateful implementation uses Gemini 3.8 Flash high through agy CLI as economy-worker; fresh independent review uses Kimi K3 256K through Kimi CLI as economy-reviewer. A verified economy-worker blocker may consume the separately bounded one-run Astra high diagnostic reserve, then return its advice to the unchanged coordinator and same worker context. Resolve exact identifiers from `MODELS.md`; no silent provider, model, effort, native-subagent, or premium substitution. No other premium use without configured reserve. One bounded milestone, at most two implementation writers, no nested tournaments, only concrete authorized delegation, no status relaying.
