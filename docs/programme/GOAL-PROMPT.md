...
- Planner, worker, and reviewer execute through their installed external CLIs and must never be replaced with native Codex subagents. Ordinary coordination remains `gpt-5.6-sol` at medium effort.
...
Read current `AGENTS.md`, mandatory Antidote instructions, Task Observer session-start, `/home/thetu/.codex/workflows/economy/POLICY.md`, and `MODELS.md`. Exact roles: ordinary coordination uses Codex `gpt-5.6-sol` at medium effort; planning uses Fable 5.1 through Claude CLI; mechanical/stateful implementation uses Gemini 3.8 Flash high through agy CLI as economy-worker; fresh independent review uses Kimi K3 256K through Kimi CLI as economy-reviewer. A verified economy-worker blocker may consume the separately bounded one-run Astra high diagnostic reserve, then return to coordinator and same worker context. Resolve exact identifiers from `MODELS.md`; no silent provider, model, effort, native-subagent, or premium substitution. No other premium use without configured reserve. One bounded milestone, at most two implementation writers, no nested tournaments, only concrete authorized delegation, no status relaying.
