...
| **S2.3:** a ledger commit followed by a lost response reconciles exactly once | `pending-ops.test.ts`: `resolved create claim survives delivery ack and identical create reuses stored result with one POST`; unresolved claim retention; host `getPendingExecutionClaims` execution command reconciliation (`seal_execution_criteria`, `stamp_execution_plan`, `set_execution_state`) with authoritative stored-operation resolution, canonical request-hash verification, and principal isolation; type mismatch rejection. `test_workflow_service.py`: `test_receipt_idempotency_exact_retry_returns_original_row`, `test_completion_evidence_idempotency_and_claim_race`. Execution smoke's `crash-gap` scenario, near line 1347, writes a synthetic resolved claim after a successful command. Installed `test_installed_execution_recovery.py`: `test_committed_criteria_post_response_loss_reconciles_same_operation`, `test_committed_plan_stamp_post_response_loss_reconciles_same_operation`, `test_committed_resume_post_response_loss_reconciles_same_operation`. Commands **P**, **W**, **E**, **I**. | Installed qualification in `test_installed_execution_recovery.py` covers committed mutation response-loss boundaries across `seal_execution_criteria`, `stamp_execution_plan`, and paused-resume `set_execution_state`: forwards each real command to the actual WorkService, withholds the committed response with zero response bytes delivered, kills the issuing CLI process, and restarts from the client-written unresolved pending claim. The restarted CLI resolves the original genuine claim via authoritative GET lookup with matching canonical request hash, issuing zero replay POSTs and advancing to the expected phase or active execution grant with matching stamp or version. Note that CI scenario upload sufficiency in `.github/workflows/ci.yml` is explicit to named artifact globs (`authority-*.json`, `setup.json`, `controller-*.stderr`, `first-run-*`, etc.) rather than unindexed scenario-specific json artifacts. Remaining execution mutation classes (`activate_execution_item`, `complete_execution_item`) and other fault phases remain unqualified residuals. Command **I** must run against the candidate-bound release; this does not claim native acceptance, live deployment, paid-model autonomy, or full OMP-246 completion. |
