...
| **S2.3:** a ledger commit followed by a lost response reconciles exactly once | `pending-ops.test.ts`: `resolved create claim survives delivery ack and identical create reuses stored result with one POST`; unresolved claim retention; host `getPendingExecutionClaims` execution command reconciliation covers exactly qualified set (`seal_execution_criteria`, `stamp_execution_plan`, `set_execution_state`, and since OMP-246 `complete_execution_item`) with authoritative stored-operation resolution, canonical request-hash verification, and principal isolation; type mismatch rejection; unadmitted begin/activate claims untouched and unfetched while a committed completion claim is resolved (`getPendingExecutionClaims leaves unadmitted begin/activate claims untouched and unfetched while reconciling a committed complete_execution_item claim`); queue lost-completion recovery at owned session start (`omp246-lifecycle.test.ts`: one operation GET, one activation at the post-completion version, one continuation, no duplicate on a second restart); pre-upgrade missing reason claim refused as identity mismatch (`pre-OMP-277 set_execution_state claim without reason key is refused as identity mismatch`). `test_workflow_service.py`: `test_receipt_idempotency_exact_retry_returns_original_row`, `test_completion_evidence_idempotency_and_claim_race`. Execution smoke's `crash-gap` scenario, near line 1347, writes a synthetic resolved claim after a successful command. Installed `test_installed_execution_recovery.py`: `test_committed_criteria_post_response_loss_reconciles_same_operation`, `test_committed_plan_stamp_post_response_loss_reconciles_same_operation`, `test_committed_resume_post_response_loss_reconciles_same_operation`. Commands **P**, **W**, **E**, **I**. | Installed qualification in `test_installed_execution_recovery.py` covers committed mutation response-loss boundaries across `seal_execution_criteria`, `stamp_execution_plan`, and paused-resume `set_execution_state`: forwards each real CLI commit to the actual WorkService, withholds the committed response with zero response bytes delivered, kills the issuing CLI process (SIGKILL), restarts in the same session (restart 1) to resolve the genuine claim via authoritative GET lookup with matching canonical request hash, issues zero replay POSTs and advances to the expected phase or active execution grant with matching stamp or version, exercises repeated resume prompts, kills the first restarted controller (second SIGKILL), and starts a third process (restart 2) within enclosing service/proxy scopes. Restart 2 proves unchanged post-first-restart versions, database rows, receipt uniqueness, single continuation for recovered postVersion, zero replay POSTs, and zero operation re-fetches (evidence `authority-committed-second-restart.json`). Residual A (unqualified/unadmitted): `begin_execution` and `activate_execution_item` pending claims are skipped by startup reconciliation (preserving base behavior), because the client omits defaulted keys (`project_id`, `expected_project_id`, `expected_blocker_ids`) preventing client-side canonical request hash verification; fixing this requires required-key payload types and producer normalization in `packages/work-client` (outside this child scope). `complete_execution_item` is reconciled since OMP-246 (its client payload emits every model key), but it still has no whole-process installed case: the mock-service package proves the host transition, not native request-hash parity. Residual B (pre-upgrade compatibility): a `set_execution_state` claim written by a pre-OMP-277 client (no `reason` key) still pending at upgrade hashes without `reason` and is refused as identity mismatch (base: silently skipped); exit is stop/cancel or manual claim repair; revert-only rollback is clean (service accepts `reason: null` per models.py:875; older clients ignore non-seal claims). CI scenario upload sufficiency in `.github/workflows/ci.yml` is explicit to named artifact globs (`authority-*.json`, `setup.json`, `controller-*.stderr`, `first-run-*`, etc.) covering the second restart artifacts. Command **I** must run against the candidate-bound release; this does not claim native acceptance, live deployment, paid-model autonomy, parent OMP-246 completion, or measured net benefit. |
...
| **S2.10:** retained terminal predecessor edges are nonblocking; completion preserves validated atomic transitions | One service observer reads sorted incoming source IDs and locks source states `FOR SHARE` at admission, activation and both completion routes. Eligibility uses this transaction’s fully validated terminal post-state, otherwise current state: `complete_work` projects validated cancellation to `CANCELED` and validated satisfied-child/sealed-rider completion to `DONE`, after all proofs and before the first completion write. `complete_execution_item` has no such batch writes: it remains strict current-state and never projects riders `DONE`. Only exact `DONE`/`CANCELED`/`CANCELLED` states qualify; execution source IDs must still equal the immutable seal. Host recovery performs a final keyed predecessor read with matching `work_id` and returns visible retryable read refusals. Real PostgreSQL service cases cover retained edges, proof-first ordering, replay, source/state drift and bounded concurrency; commands **W**, **E**. | Installed cases in `test_installed_execution_recovery.py` cover startup and paused-resume keyed-read fault/restore, supported canceled-to-backlog reopen, retained edges and same-grant recovery. Completion requests are driver-issued against the actual installed service/PG after controller recovery; they are not claimed as controller-issued completion. Command **I** must still run against the candidate-bound release; this map claims no installed PASS. Schema-impossible dangling predecessor and malformed/missing immutable-seal branches remain defensive/uncovered rather than forged through disabled constraints. Preserve historical edges, existing fingerprint/authority checks and separate native acceptance/live qualification. |
