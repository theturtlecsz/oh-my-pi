...
describe("isCommonJsModulePath CJS classification (inheritedKind override fix)", () => {
	...
	it("classifies defineProperty-only CJS emit (rollup pattern) imported from ESM as CJS", async () => {
		const dir = await writePackage({
			"package.json": JSON.stringify({ name: "cjs-defineproperty-esm", version: "1.0.0" }),
			// Rollup-style CJS emit: every export rides Object.defineProperty(exports, …);
			// no `exports.x =` member assignment and no require() call ever appears.
			"dep.js": [
				"Object.defineProperty(exports, '__esModule', { value: true });",
				"Object.defineProperty(exports, 'answer', { get() { return 42; } });",
			].join("\n"),
			"index.mjs": ["import dep from './dep.js';", "export const answer = dep.answer;"].join("\n"),
		});

		const entry = path.join(dir, "index.mjs");
		const mod = (await loadLegacyPiModule(entry)) as { answer: number };
