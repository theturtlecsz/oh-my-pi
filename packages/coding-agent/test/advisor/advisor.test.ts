...
describe("advisor", () => {
	...
	describe("AdviseTool", () => {
		...
		it("defers non-blockers during in-progress updates and flushes them on the next completed update", async () => {
			...
			const deferred = await tool.execute("tc-1", { note, severity: "concern", category: "semantic-concern" });
			...
			expect(onAdvice).toHaveBeenCalledTimes(3);
			expect(onAdvice).toHaveBeenNthCalledWith(2, note, "concern", "semantic-concern");
			expect(onAdvice).toHaveBeenNthCalledWith(3, "Minor naming cleanup.", "nit", "semantic-concern");
		...
		it("does not pile up duplicate deferred notes during a long mid-turn", async () => {
			...
			await tool.execute("tc-2", { note, severity: "concern", category: "semantic-concern" });
			await tool.execute("tc-3", { note, severity: "concern", category: "semantic-concern" });

			tool.beginUpdate(false);
			// Identical note queued once, flushed once.
			expect(onAdvice).toHaveBeenCalledTimes(1);
			expect(onAdvice).toHaveBeenCalledWith(note, "concern", "semantic-concern");
		});

		it("retains the highest severity and category when duplicate deferred advice escalates", async () => {
			const onAdvice = vi.fn();
			const tool = new AdviseTool(onAdvice);

			tool.beginUpdate(true);
			await tool.execute("tc-1", {
				note: "Same point raised repeatedly.",
				severity: "nit",
			...
			tool.beginUpdate(false);
			expect(onAdvice).toHaveBeenCalledTimes(1);
			expect(onAdvice).toHaveBeenCalledWith("Same point raised repeatedly.", "concern", "gate-defect");
		});
