...
def test_loopback_service_replays_real_postgres_commands_and_enforces_capabilities(tmp_path: Path) -> None:
    ...
    with native_postgres(tmp_path, config.port):
        ...
        (capabilities / "reader.json").write_text(json.dumps({"token": "reader-token", "actor_id": str(uuid4()), "actor_kind": "task-agent", "workspaces": [str(workspace_id)], "scopes": ["work.candidate.read"], "candidate_ids": [str(uuid4())]}))
