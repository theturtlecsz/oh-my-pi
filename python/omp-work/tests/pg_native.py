...
def seed_authority(dsn_kwargs: dict, workspace_id: UUID, actor_id: UUID) -> None:
    ...
    with psycopg.connect(**dsn_kwargs, autocommit=True) as connection:
        connection.execute("SELECT set_config('omp.workspace_id', %s, false), set_config('omp.actor_id', %s, false)", (str(workspace_id), str(actor_id)))
        if connection.execute("SELECT 1 FROM omp_control.workspace_authority WHERE workspace_id=%s", (workspace_id,)).fetchone():
            return
        connection.execute(
        ...
        connection.execute("INSERT INTO omp_control.workspace_authority(workspace_id,epoch_id) VALUES(%s,%s)", (workspace_id, epoch_id))
