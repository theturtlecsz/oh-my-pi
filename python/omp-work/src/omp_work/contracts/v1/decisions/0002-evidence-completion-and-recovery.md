...
`pushed_branch` completion requires current-candidate plan evidence, verification, explicit independent `PASS` audit, and a remote-observed push receipt resolving to the candidate commit, or to a same-branch tip that provably contains it (OMP-99). Completion is recorded only after those checks. Missing or mismatched evidence leaves closeout intent recoverable and the work non-DONE.
