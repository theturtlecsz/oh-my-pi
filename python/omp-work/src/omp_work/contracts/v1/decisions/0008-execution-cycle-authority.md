...
2. **Owner-only scope `work.execute` and commands.** WorkService exposes seven commands under scope `work.execute`:
   ...
   - `complete_execution_item`: Consumes the required `evidence: CompletionEvidence` claim (OMP-247), validates all subject, runner, check, artifact (verification, audit, push), and delivery bindings against service-owned rows, and never selects an unreferenced audit or push receipt; mints closeout receipt, completes work item to `DONE`, completes grant item, and completes grant when all items are done.
   - `skip_active_item` (OMP-219): owner deferral of the grant's active item — supersedes any open close attempt, leaves the work item open (never completed or canceled), and advances the grant to its next item or completes it without spending a close-attempt budget slot.
