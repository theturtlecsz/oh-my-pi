-- OMP-69: a host-side auditor spawn failure abandons its reservation without
-- consuming the bounded launch budget. Immutable launch rows remain history;
-- launch_count is the reservation sequence and cancelled_launch_count makes
-- the effective budget explicit.
ALTER TABLE omp_work.close_attempts
  DROP CONSTRAINT close_attempts_launch_budget_check,
  DROP CONSTRAINT close_attempts_report_budget_check,
  ADD COLUMN cancelled_launch_count integer NOT NULL DEFAULT 0,
  ADD CONSTRAINT close_attempts_launch_budget_check CHECK (launch_count - cancelled_launch_count BETWEEN 0 AND 3),
  ADD CONSTRAINT close_attempts_cancelled_launch_check CHECK (cancelled_launch_count BETWEEN 0 AND launch_count),
  ADD CONSTRAINT close_attempts_report_budget_check CHECK (accepted_report_count BETWEEN 0 AND 2 AND accepted_report_count <= launch_count - cancelled_launch_count);

ALTER TABLE omp_work.auditor_launches DROP CONSTRAINT auditor_launches_launch_number_check;
ALTER TABLE omp_work.auditor_launches ADD CONSTRAINT auditor_launches_launch_number_check CHECK (launch_number >= 1);
