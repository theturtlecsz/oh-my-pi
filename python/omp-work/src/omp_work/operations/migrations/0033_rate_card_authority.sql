-- Rate-card authority: WorkService-owned, immutable rate cards (OMP-233).
-- Distinguish qualified estimated prices from unknown/unpriced usage.
-- Direct table writes on omp_work.rate_cards remain denied except to omp_work_app INSERT/SELECT.

CREATE TABLE omp_work.rate_cards (
  rate_card_id uuid PRIMARY KEY,
  workspace_id uuid NOT NULL,
  provider text NOT NULL,
  version text NOT NULL,
  billing_modes text[] NOT NULL CHECK (cardinality(billing_modes) > 0 AND billing_modes <@ ARRAY['subscription', 'metered', 'purchased_credit', 'local']::text[]),
  effective_from timestamptz NOT NULL,
  effective_until timestamptz CHECK (effective_until IS NULL OR effective_until > effective_from),
  currency text NOT NULL CHECK (currency ~ '^[A-Z]{3}$'),
  unit_prices jsonb NOT NULL CHECK (jsonb_typeof(unit_prices) = 'object'),
  evidence_sha256 text NOT NULL CHECK (evidence_sha256 ~ '^[0-9a-f]{64}$'),
  evidence_source text NOT NULL,
  observed_at timestamptz NOT NULL,
  qualification text NOT NULL CHECK (qualification IN ('unqualified', 'qualified')),
  registered_at timestamptz NOT NULL DEFAULT clock_timestamp(),
  UNIQUE (workspace_id, provider, version)
);

CREATE INDEX rate_cards_workspace_provider ON omp_work.rate_cards(workspace_id, provider);

SELECT omp_control.install_workspace_rls('omp_work.rate_cards'::regclass, 'workspace_id');
REVOKE ALL ON omp_work.rate_cards FROM omp_work_app, omp_work_readonly, omp_work_importer;
GRANT SELECT, INSERT ON omp_work.rate_cards TO omp_work_app;
GRANT SELECT ON omp_work.rate_cards TO omp_work_readonly;
REVOKE UPDATE, DELETE, TRUNCATE ON omp_work.rate_cards FROM omp_work_app, omp_work_readonly, omp_work_importer;
ALTER TABLE omp_work.rate_cards FORCE ROW LEVEL SECURITY;
