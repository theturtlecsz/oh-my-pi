...
def _connect(
    config: OperationsConfig, role: str, database: str | None = None, *, autocommit: bool = False
    ...
    try:
        with _connect(config, "postgres", "postgres", autocommit=True) as conn:
            with conn.cursor() as cur:
                ...
                for role in ("omp_work_migrator", "omp_work_app", "omp_work_importer", "omp_work_readonly", "omp_work_backup"):
                    cur.execute(sql.SQL("ALTER ROLE {} PASSWORD {}").format(sql.Identifier(role), sql.Literal(config.read_secret(role))))
                cur.execute("SELECT 1 FROM pg_database WHERE datname = %s", (config.database,))
                if cur.fetchone() is None:
                    cur.execute(sql.SQL("CREATE DATABASE {} OWNER omp_work_owner").format(sql.Identifier(config.database)))
    except Exception as error:
        raise _redacted(error)
    migrate(config)


def check_migrations(conn: psycopg.Connection[Any], *, allow_pending: bool) -> dict[str, list[str]]:
    expected = migrations()
    with conn.cursor() as cur:
        cur.execute("SELECT ordinal, filename, sha256 FROM omp_control.schema_migrations ORDER BY ordinal")
        actual = [(int(row[0]), row[1], row[2]) for row in cur.fetchall()]
    expected_rows = [(ordinal, path.name, sha256(path.read_bytes()).hexdigest()) for ordinal, path in expected]
...
def migrate(config: OperationsConfig, target: int | None = None, lock_timeout: int = 30) -> None:
    validate_bundle(require_approval=True)
    try:
        with _connect(config, "omp_work_migrator") as conn:
            with conn.transaction():
                with conn.cursor() as cur:
                    ...
                    try:
                        cur.execute("SELECT pg_advisory_xact_lock(hashtextextended(%s, 0))", (MIGRATION_LOCK,))
                    ...
                    if cur.fetchone()[0] is None:
                        state = {"pending": [path.name for _, path in migrations()], "drift": []}
                    else:
                    ...
                    for ordinal, path in migrations():
                        ...
                        cur.execute(
                            ...
                            (ordinal, path.name, sha256(path.read_bytes()).hexdigest(), CONTRACT_VERSION, contract_sha256(), POSTGRES_MAJOR),
                        )
                    cur.execute("SELECT to_regclass('omp_control.runtime_compatibility')")
                    if cur.fetchone()[0]:
                        cur.execute("INSERT INTO omp_control.runtime_compatibility (contract_version, contract_sha256, migration_set_sha256, postgres_major) VALUES (%s,%s,%s,%s) ON CONFLICT (singleton) DO UPDATE SET contract_version=EXCLUDED.contract_version, contract_sha256=EXCLUDED.contract_sha256, migration_set_sha256=EXCLUDED.migration_set_sha256, postgres_major=EXCLUDED.postgres_major", (CONTRACT_VERSION, contract_sha256(), migration_set_sha256(), POSTGRES_MAJOR))
    except ValueError:
        raise
    except Exception as error:
        raise _redacted(error)
...
def collect_health(config: OperationsConfig, role: str = "omp_work_migrator") -> HealthReport:
    report = HealthReport()
    try:
        with _connect(config, role) as conn:
            with conn.cursor() as cur:
                ...
                report.live = True
                report.postgres = {"major": version // 10000, "in_recovery": recovery, "read_only": read_only == "on"}
                try:
                ...
                cur.execute("SELECT contract_version, contract_sha256, migration_set_sha256, postgres_major FROM omp_control.runtime_compatibility")
                compatible = cur.fetchone() == (CONTRACT_VERSION, contract_sha256(), migration_set_sha256(), POSTGRES_MAJOR)
                if not compatible:
                ...
                evidence = {
                    row[0]: {"outcome": row[1], "ended_at": row[2], "byte_count": row[3], "duration_seconds": row[4]}
                    for row in cur.fetchall()
                }
                report.backup = evidence.get("backup", {})
                report.restore = evidence.get("restore_drill", {})
                report.wal = {"current_lsn": None}
                cur.execute("SELECT pg_current_wal_lsn()::text, pg_wal_lsn_diff(pg_current_wal_lsn(), '0/0')::bigint, pg_database_size(current_database())")
                current_lsn, wal_bytes, database_bytes = cur.fetchone()
                report.wal = {"current_lsn": current_lsn, "bytes_since_init": wal_bytes, **evidence.get("wal_upload", {})}
                ...
                try:
                    with conn.transaction():
                        cur.execute("INSERT INTO omp_control.readiness_probe DEFAULT VALUES ON CONFLICT (singleton) DO UPDATE SET checked_at = clock_timestamp()")
                        raise _RollbackProbe()
                except _RollbackProbe:
                    pass
                report.ready = not any(
                    alert in {"MIGRATION_DRIFT", "POSTGRES_RECOVERY", "POSTGRES_READ_ONLY", "MIGRATION_PENDING"}
                    for alert in report.alerts
                ) and version // 10000 == POSTGRES_MAJOR and compatible
    except Exception:
...
def check(config: OperationsConfig) -> dict[str, Any]:
    ...
    return {"postgres": version, "contract": CONTRACT_VERSION, "contract_sha256": contract_sha256(), "migration_set_sha256": migration_set_sha256(), **migration}
