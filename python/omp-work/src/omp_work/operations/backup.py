...
def provision_target(config: OperationsConfig) -> None:
    ...
    existing = _aws(config, ["s3api", "list-buckets", "--query", f"Buckets[?Name=='{config.bucket}'].Name", "--output", "text"]).strip()
    if existing:
        raise ValueError("backup target already exists")
    _aws(config, ["s3api", "create-bucket", "--bucket", config.bucket])
    _aws(config, ["s3api", "put-bucket-versioning", "--bucket", config.bucket, "--versioning-configuration", "Status=Enabled"])
    ...
    _aws(config, ["s3api", "put-bucket-lifecycle-configuration", "--bucket", config.bucket, "--lifecycle-configuration", lifecycle])
...
def create(config: OperationsConfig) -> str:
    ...
    try:
        ...
        artifacts = [dump]
        _run(["pg_dump", "--host", config.host, "--port", str(config.port), "--username", "omp_work_backup", "--format=custom", "--no-owner", "--no-privileges", "--file", str(dump), config.database], env=_postgres_env(config))
        _run(["pg_basebackup", "--host", config.host, "--port", str(config.port), "--username", "omp_work_backup", "--format=tar", "--gzip", "--wal-method=stream", "--pgdata", str(physical_dir)], env=_postgres_env(config))
        ...
        for artifact in artifacts:
            ...
            digest = encrypt_file(artifact, encrypted, config.secret_path("gpg-passphrase"))
            ...
            _aws(config, ["s3api", "put-object", "--bucket", config.bucket, "--key", key, "--body", str(encrypted), "--metadata", metadata])
            _aws(config, ["s3api", "head-object", "--bucket", config.bucket, "--key", key])
            objects.append((key, digest))
        manifest = staging / "manifest.json"
        manifest.write_text(json.dumps({"backup_id": backup_id, "contract_sha256": contract_sha256(), "migration_set_sha256": migration_set_sha256(), "objects": [{"key": key, "sha256": digest} for key, digest in objects]}) + "\n")
...
def upload_wal(config: OperationsConfig) -> int:
    ...
    for segment in spool.iterdir():
        ...
        try:
            ...
            _aws(config, ["s3api", "put-object", "--bucket", config.bucket, "--key", key, "--body", str(encrypted), "--metadata", f"sha256={digest}"])
            _aws(config, ["s3api", "head-object", "--bucket", config.bucket, "--key", key])
            segment.unlink()
            uploaded += 1
        finally:
...
def _download_decrypt(config: OperationsConfig, key: str, destination: Path) -> None:
    ...
    _aws(config, ["s3api", "get-object", "--bucket", config.bucket, "--key", key, str(encrypted)])
...
def restore_drill(config: OperationsConfig, *, reason: str) -> str:
    ...
    try:
        ...
        if payload["contract_sha256"] != contract_sha256() or payload["migration_set_sha256"] != migration_set_sha256():
            raise RuntimeError("backup compatibility mismatch")
        dump = staging / "ledger.dump"
        dump_key = next(item["key"] for item in payload["objects"] if item["key"].endswith("/ledger.dump.gpg"))
