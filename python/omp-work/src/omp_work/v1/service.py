...
class WorkService:
    _scopes = {
        ...
        "append_evidence": "work.approve", "finalize_candidate": "work.approve", "create_same_session_child": "work.close", "begin_close_attempt": "work.close", "seal_audit_manifest": "work.close", "reserve_auditor_launch": "work.close", "cancel_auditor_launch": "work.close", "settle_auditor_launch": "work.close", "attest_checkpoint_delivery": "work.close", "record_closeout_review": "work.close", "complete_work": "work.close", "stage_import_batch": "work.import", "promote_import_batch": "work.import", "activate_cutover": "work.operate", "attest_cutover_plan": "work.operate",
    }
    ...
    def read(self, principal: Principal, workspace_id: UUID, kind: str, value: str) -> dict[str, object]:
        ...
        if "work.read" not in principal.scopes:
            if "work.candidate.read" not in principal.scopes or not principal.candidate_ids or kind != "workflow":
                raise WorkError("forbidden", status=403)
            allowlist = principal.candidate_ids
        try:
            return self._store.read(workspace_id, principal.actor_id, kind, value, candidate_allowlist=allowlist)
        except WorkStoreError as error:
            statuses = {"invalid_request": 400, "forbidden": 403, "unavailable": 503}
            raise WorkError(error.code, status=statuses.get(error.code, 409), diagnostics=error.diagnostics) from error
