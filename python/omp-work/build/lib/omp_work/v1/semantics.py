...
from .models import Candidate, CompletionBlocker, CompletionInput, EvidenceKind, EvidenceReceipt, RelationEdge, RelationKind
...
def completion_blockers(input: CompletionInput) -> tuple[CompletionBlocker, ...]:
    ...
    if not any(receipt.kind is EvidenceKind.AUDIT and receipt.independent and receipt.verdict == "PASS" for receipt in fresh):
        blockers.append(CompletionBlocker(code="audit_missing", detail="current candidate lacks an independent PASS audit"))
