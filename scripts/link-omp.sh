...
#
# Directly installs the source-development `omp` wrapper at
# `packages/coding-agent/scripts/omp` into Bun's global bin directory without
# requiring Bun's global package registry. See that wrapper's header comment
# for the bunfig.toml-preload bug it works around.
#
