Status moved to the Work Ledger on 2026-09-25. Checkpoint and status sections below are frozen history. This file is the programme specification.
...
qualification used disposable PostgreSQL, staged launcher/service, candidate daemon/Web build, RPC
controller and real headless Chromium: **2 pass, 0 fail/skip in 74.83s**. It proves pause with
committed response loss and durable reconciliation, stale concurrent stop refusal, worker drain,
terminal stop/late-result fencing, SIGKILL, same-session terminal restart, daemon loss and reconnect,
plus negative no-completion/no-version-mutation effects. Per-step HAR/JUnit evidence and state are
...
E1 PASS lineage and reviewed E2 ECC lineage are integrated at local merge commit
`a06663f0c47b197b4c57931d25671ff3e302eed9`; staging repair commit
`22a1ddaf91853db80141e2cb5c9b40d53cd799d0` materializes exact clean gitlinks. Conflict resolution retained explicit native
role activation with E2 recipes/accounting/advisor surfaces. Integrated verification: **212
TypeScript pass**, **5 real-PostgreSQL pass**, staging **10 pass**, `bun check` exit 0 with three known warnings.
[Checkpoint](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/E1-E2-INTEGRATION-CHECKPOINT.md).
Installed release composition now reaches candidate `ff66695e…`, manifest `b64a3851…`: exact
submodule commit/tree/files are materialized and verified from externally anchored release identity,
and Web qualification binds ignored dist bytes. Pilot-r4 tasks 1–3 now have accepted typed records:
installed browser controls pass 2/2, OMP-272 passes 191 tests and 1,233 assertions, and WP5 forced
retry accounting passes an installed one-503/two-transport qualification with stage and WorkService
request counts both equal to 2, unknown request usage retained and price unpriced. Task 3 output
commit `72b94da2996da87435c019eaedea8b095ead3abd` was staged as immutable r12 manifest
`decf2653…`; legacy installed completion and browser Pause/Stop siblings also pass. Independent Kimi
R2 returned PASS after its R1 outer-retry double-count finding was repaired. [Pilot checkpoint](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/pilot-r4/CHECKPOINT.md).
Pilot task 4 now also passes: exact database-advisor pack applied in staged r14, exposed only
`advise/read/glob/grep`, read repository schema evidence, deduped duplicate findings, accounted
33 input/21 output tokens, fenced a held late response after `/advisor off`, and reports disabled
status accurately. Relevant advisor/ECC suites pass 158 tests; independent Kimi R2 returned PASS.
Pilot task 5 now also passes: OMP-280 pagination is structurally bounded, duplicate full pages and
cap exhaustion fail before write, and six pack-conditioned loopback HTTP scenarios preserve
absence, all-present, creation, provider-error and exact-URI behavior. Combined changed scope is
15 pass; independent Kimi returned PASS. Mandatory full gate passed every isolated partition and
then hit the known unchanged Pluto live-data assertion in corpus collection; failure is retained.
Final no-pack WebUI task passes 2/2 installed browser scenarios against stable r9: stale target,
committed response loss, bounded reason, terminal no-op, daemon loss/same-port reconnect, worker
fencing and browser secret exclusion. Daemon suites pass 30 and Web Vitest passes 33; independent
Kimi returned PASS. Pilot r4 validator now returns **PASS, 6/6 accepted records**.
[Pilot result](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/PILOT-R4-RESULT.md).
Prior setup failures and rejected stale-dist evidence remain historical and do not become r4
successes. Screening pilot is complete; integrated ECC activation, WP5C/D and WP7/8 remain open.
No push, main merge, live install or native mutation.

E2 cumulative source checkpoint: reviewed WP1/2 adapter and apply/remove, WP3 pack/pilot contract,
WP4 typed audit, WP5A usage receipt and WP5B frozen recipes now form local candidate commit
`c8f5263cfa8f6c949dc84d380b8ac05192dbbe26`, tree `d51ef5c3…`. Fresh cumulative evidence:
TypeScript **283 pass**, PostgreSQL/API **36 pass**, `bun check` and schema gates exit 0; three
known unchanged warnings. [Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/CUMULATIVE-INTEGRATION-EVIDENCE.md).
Six existing-scope pilot tasks are preregistered at digest `a5613cfb…`; validator exit 0 with zero
records. [Pilot preflight](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/pilot-r1/PREFLIGHT.md).
No task ran and no benefit claim exists. WP5C/D, WP6–8, installed activation, complete E2 and later
programme phases remain open. Commit stayed local; no push/merge/release/live effect.

WP7A bounded source slice independently accepted: evidence-bound proposals now carry explicit
trigger, proposed action and verified source-task metadata; support reads expose contradictory
outcomes; optional lesson retrieval has an opt-in newest-first record cap alongside exact byte
budgeting. Mandatory authority/evidence and engine facts remain outside the cap, cap drops stay
visible, configured caps bind bundle identity and uncapped identity bytes remain compatible. Real
PostgreSQL focused suites pass 48 tests, bridge/cross-language passes 117 tests / 483 assertions,
and `bun check` exits 0 with three known warnings. Kimi R1 found test gaps only; R2 returned PASS
after cap0, cap-vs-byte, mismatch, contradiction and uncapped-identity regressions landed.
[Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7A-SOURCE-EVIDENCE.md).
Source commit `d3c9d4e3e260e95966371539e822701b8eedfd57`, integrated at
`3675fd97968a8f7cf92fe9ea576d599eb9cfb402`.

WP7A live producer is now independently accepted. Explicit typed `learn.proposal` records stage
only after exact independent PASS audit receipts; malformed, failed, non-independent,
NEEDS_FIX/BLOCKED and candidate-mismatched observations fail closed. Evidence binds receipt ID/hash
and source observation; 4xx responses become terminal withheld markers, while 5xx/network failures
persist exact request bytes plus task identity for restart replay. Authenticated observation reads
are bounded, exact-identity, withdrawn-row excluding and indexed. Proposal identity now ignores
optional enrichment health. TypeScript suites pass **96 tests**, real-PostgreSQL focused suites pass
**20 tests**, `bun check` and diff checks pass; Kimi R2 returned PASS after required negative-path
coverage landed. Source commit `322491491b52c306467ffd66ef36a543f90747a2`, tree
`9596e476c637dab0d927f892c537fa07b13e2a77`, integrated at local merge
`bd3f81eac81ceec26636591d05c8a489ae4c14e4`.
[Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7A-PRODUCER-SOURCE-EVIDENCE.md).
WP7A exact token budgets are now independently accepted. Active model tokenizer identity resolves
through catalog policy into the native counter; unknown tokenizer families fail closed without byte
fallback. One mode-0600 typed pin binds argv, cwd, executable, entry, selected addon, manifest and
profile digests. Compiler budgets and replay identities include exact tokenizer/profile provenance;
bridge independently recounts served canonical bytes with its own native tokenizer. Record caps
remain proposal-only, while total token limit covers mandatory plus optional content. Source and
compiled binary profiles both count the probe as 3 tokens, including compiled BunFS entry handling.
Focused PostgreSQL suites pass **26 tests**, combined bridge/cross-language/CLI/native suites pass
**189 tests**, `bun check` passes, and Kimi R2 returned PASS after contract, failure-matrix,
cap-attribution and write-deadline repairs. Source commit
`469dd201375dbdacc365b21001ed5ce7712cbcea`, tree
`1de76abc51f9517583949b33716bfe1a8b93ffc2`, integrated at local merge
`9bdf549a6eb056db021f9a2363bbb53c3c2b539e`.
[Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7A-TOKEN-EVIDENCE.md).
Installed pin provisioning, OpenAI/Gemini tokenizer coverage, retrieval-quality qualification and
promotion remain open; WP7A/WP7/E2 are not complete. No push, main merge, live install, service
restart or native mutation.

WP7B native-event inventory is complete. Existing OMP session start/switch, tool-result capture,
compaction/plan protection, WorkService recovery, terminal trace, session-ledger and shutdown paths
already cover four lifecycle rows more safely than ECC's duplicate session files, blanket checks,
observer scripts and automatic LLM summaries; those raw hooks are rejected for runtime import. One
source gap remains: exact user-attributed `skill-prompt` invocation identity is not durably linked
to later independent audit outcome. Fable planning selects additive `skill_invocation` and
`skill_outcome` observations, existing durable capture/replay and exact receipt verification, with
no new table, daemon, prompt, model loop or authority path.
[Inventory](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7B-NATIVE-EVENT-INVENTORY.md).
[Plan](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7B-SKILL-OUTCOME-PLAN.md).
WP7B skill outcome linking is now independently accepted. Exact user-attributed `skill-prompt`
messages produce candidate-bound invocation observations containing prompt fingerprints and bounded
argument provenance, never expanded prompt bytes. Outcomes append only after pre-admission
verification against the exact independent WorkService audit receipt and matching invocation/task
identity. PASS, NEEDS_FIX and BLOCKED remain distinct receipt evidence; a non-error invocation never
implies success. Existing durable capture/replay handles restart and cross-session readback without
new storage, prompts, model loops, promotion, grants or closeout authority. Focused PostgreSQL suites
pass **33 tests**, bridge suites pass **75 tests**, `bun check` passes, and Kimi returned PASS after
independently reproducing all gates. Source commit
`502c4b4b991b19c1adb867564ab70d37d34e58b4`, tree
`e421ae74c0211a9485c4e548dc99a2387afd5ab8`, integrated at local merge
`7ef0e899f49e215f24e1727f604087be07d81a93`.
[Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP7B-SKILL-OUTCOME-SOURCE-EVIDENCE.md).
Integrated WP7 staged-release component qualification is independently accepted. Exact merge
`7ef0e899…` was packaged as immutable release manifest `245e7e94…`. Its release-bound token counter
reproduced exact profile hashes and counts across process launches; staged PostgreSQL compiler
qualification passed **1 test**, staged skill observation/receipt verification passed **7 tests**,
and exact staged bridge passed **75 tests / 369 assertions**. Kimi independently reverified all
48,746 manifest entries, reran launcher/compiler/service/bridge checks and returned **PASS**. Review
found pytest cache pollution after initial staging; fail-closed launcher inventory detected it,
non-manifest cache bytes were removed, full inventory and launcher rerun passed, and future procedure
now disables pytest/bytecode caches before final inventory verification.
[Result](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp7-installed-qualification/RESULT.md).
[Review](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp7-installed-qualification/KIMI-REVIEW.md).
Full installed WP7B single-process skill prompt→Knowledge→real WorkService auditor-settlement journey,
later-task use/correction and any NOW fallback decision remain open; no live install, service restart
or native mutation.

WP8 AgentShield read-only diagnostic is independently accepted for its bounded claim. Pinned
`ecc-agentshield@1.6.0` tarball integrity matches npm metadata and published entry bytes exactly
match unsigned annotated tag `v1.6.0` at source commit `b0891303…`. Actual OMP-derived agent,
auditor, skill and hook bytes were scanned in control and seeded fixtures. The seeded fixture added
26 distinct findings and detected every planted wildcard permission, secret, remote-shell MCP,
malicious agent and persistence-hook defect; built-in corpus detected 11/11 attacks. AgentShield
SARIF imported through OMP's existing typed security importer with all 50 seeded findings. Control
still produced 28 findings including four critical context-insensitive prompt-defense rows and did
not establish native WorkService/grant/receipt coverage. Decision: **diagnostic qualified; enforced
gate rejected**. No auto-fix, sandbox, active injection, model analysis, project dependency, global
install or live configuration change.
[Result](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-agentshield-qualification/RESULT.md).
[Review](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-agentshield-qualification/KIMI-REVIEW.md).
WP8 maintained update procedure, production promotion, installed qualification and the 20-trial
stabilization streak remain open.

WP8 explicit upstream maintenance operation is independently accepted. Current ECC pin
`8321021c…` was compared to exact fetched candidate `07756cee…`, a descendant with 118 changed
paths and 5,684/1,128 insertions/deletions. All **13/13 selected asset sources**, all three
`skill-stocktake` helper dependencies, and all **5/5 imported adapter seams** are byte-identical;
candidate hook/bootstrap/package changes are unconsumed and inactive. Decision: **no-op update,
retain current pin**. Merging unrelated upstream churn would add risk without capability. Maintained
`update.sh` contract passes **21/21 tests**, including exact-commit/review pins, dirty/main/conflict
refusals, ordered gates, live-mapping fence and never-push/no-live-install behavior. No manifest,
submodule checkout, merge, transformed output, install, build or release changed.
[Result](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-update-qualification/RESULT.md).
[Review](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-update-qualification/KIMI-REVIEW.md).

WP8 compatible installed rollback is independently accepted for its bounded claim. Immutable current
release `a54d0aaf…` / manifest `740fea86…` and last-known-good `0aed552c…` / manifest `94a5bbeb…`
carry identical contract and 32-migration identities but different service fingerprints. A real
current→rollback→current sequence used distinct release-bound state roots against one disposable
PostgreSQL 18.6 database: rollback read current-created work byte-equivalently and wrote work later
read by restored current; every service became ready and all cut points had zero active/paused
grants. An injected unknown migration made rollback `ops check` and service startup refuse; exact
migration inventory was restored and current final check passed. Kimi independently reran the drill,
matched result bytes and returned **PASS**. No down-migration claim: different contract/migration
identities remain blocked, and old grants are never reused.
[Result](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-rollback-qualification/RESULT.md).
[Review](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/wp8-rollback-qualification/KIMI-REVIEW.md).
Production promotion, behavioral qualification and stabilization streaks remain open.

WP6 source candidate independently accepted: upstream database reviewer now maps to one explicit project-scoped
native advisor profile, reusing existing OMP advisor lifecycle rather than adding a runtime.
Read-only tools only; executable diagnostics/model binding removed; existing WATCHDOG ownership
refuses safely. Final frozen tree `881a0ac1…`, delta `bede896f…`; adapter and native advisor suites
**241 pass**, post-repair adapter/install **54 pass**, `bun check` exit 0, deterministic twin builds
and lock verification pass. Fresh Kimi K3 256K same-session review and repair-delta review returned
final PASS.
[Evidence](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP6-SOURCE-EVIDENCE.md).
[Review](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e2-ecc-20260918/WP6-INDEPENDENT-REVIEW.md).
Scoped local commit `fbf125536eb7fef06d788f9fd06153146c3cd192`; no push. Actual database-task
trial remains required; no advisor was activated and WP6/E2 remain incomplete.

NSI-1 planning gate attempted on clean E3 branch: exact Fable 5.1 session completed no plan on the
initial bounded process or same-session continuation; second turn emitted SEP-2352 credential
issuer-isolation warning. Repository stayed byte-identical. [Disposition](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e3-nsi-20260919/NSI1-PLANNING-DISPOSITION.md).
No third planner/fallback or NSI source edit; Fable architecture gate remains open. Independent
source work continues without treating this as programme blockage.

E4 reviewed source preserved: existing R02 chain contains component-compatibility commit
`d49004617e`; R04 artifact custody repaired and independently accepted at local commit
`16118ef208d6fffbb062a6e2b371abd9e9a5fe0b`, tree `a0b9bfbe…`. Direct PostgreSQL embedded-digest
mismatch test and full research contract suite pass (36 total); same Kimi reviewer returned final
PASS. [Checkpoint](/home/thetu/.codex/workflows/economy/artifacts/full-programme-e4-source-20260919/CHECKPOINT.md).
...
This side conversation records decisions and prepares documents. It does not implement product changes, alter native records, configure routes or budgets, create commits, operate parent jobs, or activate a goal. Existing production authority and Q36 remain unchanged.

| Decision | Owner direction and precise effect |
| --- | --- |
| G1 — Value | Provisional direction: useful engineering outcomes are the operating priority. Autonomous-systems research has separate value and cost. Report operating outcomes and research/development/maintenance investment separately; no invented ROI threshold or capability deletion. |
| G2 — Integration | Prioritize a usable integrated candidate, concrete approval preparation and the ordinary-task pilot. Independent authorized work may continue when blocked, while preserving integration priority. Source-slice acceptance is not installed usefulness; later phases remain required. |
| G3 — Local commits | Standing permission covers scoped local candidate commits in designated OMP programme development worktrees, preserving newer/user work and provenance. This supplies the explicit request required by repository commit rules for that scope only. Push, PR publication, merge, tags, release, activation and unrelated commits retain separate gates. No commit is performed by this amendment. |
| G4 — Best-of-N | Preserve bounded Best-of-N; adapt candidate count to task uncertainty or demonstrated benefit within the same total budget. Additional candidates must justify their cost. Neither mandatory N=3 nor mandatory N=1 is a universal default. Preserve isolated generation, independent selection, required acceptance and all discarded-work accounting. |
| G5 — Review policy | Reconcile conflicting review obligations, selecting reviewers by demonstrated quality and independence. Replacing mandatory Fable with mandatory Kimi is not itself an improvement. Current bindings and item-specific Fable publication/native Sol audit gates remain until the affected policy/criteria are explicitly reconciled through supported routes; no immediate vendor substitution or added mandatory committee. Qualification and calibrated evidence determine future assignments. |
| G6 — Uncertain inference | Permit bounded replacement only for an operation demonstrably unable to cause tool or external effects. Keep original outcome/usage unknown, preserve conservative resource holds, reserve a new attempt within the same milestone budget, and ensure at most one result is accepted downstream even after late or duplicate returns. Never relabel the original request absent/free or reuse its identity to resend. Tool-capable agents and consequential Git/database/deployment/instrument effects remain outside this exception. The capability proof, reservation and downstream arbitration must be implemented and qualified before use. |
| G7 — Intake exploration | Support bounded standing authorization for ordinary investigation within an admitted objective, permitted data/tools and meaningful resource bounds. Implement and qualify the standing-policy path; do not infer a native grant from this document. Preserve valid-pool retention, neutral presentation, cancellation and exact owner ratification of product meaning. Changed methods do not imply changed scope or adopted requirements. |
| G8 — Milestone resources | Use a known meaningful budget per milestone, shared by lead, workers, planning, review, helpers, retries, integration and recovery. Do not require per-job approvals or unavailable dollar telemetry when another meaningful authorized bound exists. Derive concrete bounds from actual entitlement/baseline evidence; this amendment assigns no numeric allowance or purchase authority. Missing telemetry stays unknown; missing applicable resource authority blocks affected spending. |
| G9 — Worker failures | Prioritize verification and repair of process ownership, completion/result collection and failure handling. Then permit a named, qualified, budgeted fallback under the same scope and cumulative allowance. A route must be named and qualified before use; no fallback vendor is selected here. Preserve repair history, reassessment after two failed repairs, no budget reset and no automatic premium substitution. Respect denied permissions; source-scope permission is not permission to disable a safety boundary. |
| G10 — Reporting | Reuse existing WorkService work/evidence reporting and MASTER links. When supported native publication is unavailable, retain an explicitly pending artifact in the existing checkpoint. Preserve upstream discovery/compatibility review without enabling GitHub issues, unsolicited issues/comments or a new tracker. |
| G11 — Completion dispositions | Completion requires each applicable obligation's evidence or explicit owner-approved removal/replacement from this goal. A disposition identifies the obligation, reason, decision, replacement/exclusion and remaining owner. Deferral or blockage leaves work outstanding. Replacing a mechanism preserves accepted capability unless its reduction is explicitly approved. |
| G12 — Finite baseline | Bind this goal to the full accepted programme represented by the September 18 audit, inherited subordinate criteria and defects necessary to satisfy them. New scope requires an explicit accepted amendment. Out-of-scope discoveries become retained successor proposals, not silent goal expansion. The 607 rows are coverage references, not independent features, effort units or the limit of inherited requirements. |

### Preservation and implementation boundaries

The complete ECC WP1–WP8, H/CPK, NSI/intake/exploration, autonomous engineering/Fleet, context/knowledge/two-repository correction, R00–R19 adaptive/bilevel/recursive research including research-method and platform improvement, and coherent WebUI/CLI/WFM remain. Predictable user interactions do not constrain useful internal reasoning, investigation or collaboration. OMP need not develop itself.

The separate 20 consecutive accepted ordinary tasks and 72 continuous hours with at least 30 distinct ordinary tasks across OMP/media and 100 actual experiment execution attempts remain, with frozen strata/recipe/evaluator, real recovery/negative cases, baseline-derived responsiveness and complete failure accounting. Existing conditional desktop/public/instrument/provider gates remain conditional, not removed or newly authorized.

WorkService retains native authority. Exact contract approval, live cutover, destructive actions, purchases, model-policy activation and withheld Q36 retain applicable gates. Prepare concrete evidence before a final owner decision; do not convert generic support into an approval record. Mandatory Observer and selected durable memory remain. Source review, integration, packaging, installed qualification, native acceptance and production activation stay distinct.

Implementation work still required: current candidate/record reconciliation; release and exact contract-delta preparation; full accounting; scoped reviewer-quality qualification and native criterion reconciliation; inference-effect classification and one-result arbitration; standing exploration enforcement; process ownership/result collection; named fallback qualification; budget baselines and enforcement; all remaining audit phases. G1's provisional framing creates no blocker to this engineering work. No new product-preference question is required before preparation; concrete authority/resource decisions remain for their actual boundaries.

Decision export: [ACCEPTED-DECISIONS.md](docs/programme/ACCEPTED-DECISIONS.md#programme-decision-amendment--september-18-2026-g1g12). Interview evidence and scope check: [programme grill](docs/report/omp-programme-grill-2026-09-18.md). Prepared, unapplied native objective: [CONTINUATION-GOAL.md](docs/programme/CONTINUATION-GOAL.md). Earlier status paragraphs retain their original dates; the parent execution checkpoint remains the runtime continuity record.
...
| --- | --- |
| Routine coordination | Code and Astra low |
...
| Premium consultation | Unavailable: no configured reserve |
