...
#     record for exactly this candidate (docs/upstream-review-<first-12-hex>.json
...
if ! git merge-base --is-ancestor "$TARGET" HEAD; then
	...
	# differ from the pin only by changes under docs/upstream-review- (guardrail
	# bookkeeping); any other divergence invalidates the review.
	REVIEW_JSON="docs/upstream-review-$(printf '%s' "$TARGET" | cut -c1-12).json"
	...
	if [ "$REVIEW_FORK" != "$HEAD_COMMIT" ]; then
		...
		DRIFT="$(git diff --name-only "$REVIEW_FORK" HEAD -- | grep -v '^docs/upstream-review-' || true)"
		if [ -n "$DRIFT" ]; then
			echo "update.sh: HEAD $HEAD_COMMIT diverges from the review's fork pin '$REVIEW_FORK' outside docs/upstream-review- — re-run the review against the current fork state" >&2
