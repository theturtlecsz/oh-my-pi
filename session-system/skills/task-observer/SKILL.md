...
3. Read `skill-observations/last-review-date.txt`. The value carries the
   ...
   Independently of the calendar: if OPEN observations cluster heavily on
   a single skill (≈5+), offer an immediate mini-review of that skill —
   clustering means the skill is actively generating failures, and waiting
   for the weekly cadence lets the same theme recur as new entries. Log
   health signals worth watching: OPEN ratio, single-skill cluster size,
   and (for logs written by parallel agents) multiple differently-named
   lock files, which indicate writers not sharing one protocol.
...
6. When a request names a target system to analyze, resolve the name to a
   concrete state root (list the candidate paths) and state the resolved
   target in one line before mining any data. An observer's default lens
   is itself; sibling harnesses with near-identical names make
   self-referential scoping the default error (#38: asked about "omx",
   the observer began analyzing its own "omc" harness).
